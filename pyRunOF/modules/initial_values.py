...
from pyRunOF.additional_fun.auxiliary_functions import Files, Priority, run_command
...
class InitialValues(Information):
    ...
    def reconstruct(self, case_path=None):
        ...
        return run_command(["reconstructPar"], case_path)
    ...
    def settingsTimeVaryingMappedFixedValue(
        self, nameSample="outletSurf", sourceTimeStep=0.25, namePatch="outlet"
    ):
    ...
    def set_mapping_settings(self, src_path, dst_path, src_time=0, dst_time=0):
        self.mapp_settings = dict(
            src_path=Priority.path(src_path, None),
    ...
    def run_mapFields(self, **options):
        ...
        * **options:
        ...
        command = getattr(self, "_commandMapFields_args", self.commandMapFields)
        return run_command(command, case_path)

    def settingsMapField(
        self,
        sourcePath=None,
        distPath=None,
        consistent=True,
        ...
        parallelSource=True,
        parallelTarget=False,
        sourceTime=0.25,
        noFunctionObjects=True,
    ):
    ...
    def createMapFieldCommand(self, option=None):
        ...
        for key in option:
            if option[key] is True:
                ...
                command.append(str(option[key]))
            elif key == "-sourceTime":
                if option[key] == "latestTime":
                    command.extend((key, "latestTime"))
                else:
                    command.extend((key, str(option[key])))
            else:
                command.extend((key, str(option[key])))
        self._commandMapFields_args = command
        self.commandMapFields = shlex.join(command)
        return self.commandMapFields

    def copyBC(
        self, nameBCsource="outlet", nameBCdist="inlet", mapTimeStep=0.25, namePostFile="outletSurf"
    ):
        """Эта функция копирует значения из postProcessing в заданном времени (mapTimeStep)
        в кейс назначения в папку constant"""

        pathScalar = os.path.join(
            self.mappSettings["sPath"],
            "postProcessing",
            namePostFile,
            str(mapTimeStep),
            nameBCsource,
        ...
        pathBCdist = os.path.join(
            self.mappSettings["dPath"], "constant", "boundaryData", nameBCdist
        )

        if os.path.exists(pathScalar):
            shutil.copytree(pathScalar, os.path.join(pathBCdist, "0"), dirs_exist_ok=True)
        shutil.copytree(pathVector, os.path.join(pathBCdist, "0"), dirs_exist_ok=True)
    ...
    def run_set_fields(self, path_case: str = None, info_key=None) -> None:
        ...
        return run_command(["setFields"], run_path)
    ...
    def calcInitVal(self, A, B, Uin, nu):
        ...
        intensity = 0.16 * Re ** (-0.125)
        L = Dh * intensity
        k = 1.5 * (intensity * Uin) ** 2
    ...
    def __checkOption(self, option):
        if option is None:
            if self.option is not None:
                ...
                raise ValueError("mapFields options must be configured")
        else:
            return option
