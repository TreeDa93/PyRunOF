from ..additional_fun.auxiliary_functions import Files, Priority, run_command
...
class Mesh(Information):
    ...
    def set_blockMesh(self, *mesh_dicts: dict, **options) -> None:
        ...
        Return: None
        """
        self._set_system_file("blockMeshDict", mesh_dicts, **options)
    ...
    def set_decomposePar(self, *mesh_dicts: dict, **options) -> None:
        ...
        Return: None
        """
        self._set_system_file("decomposeParDict", mesh_dicts, **options)

    def _set_system_file(self, file_name: str, values, **options) -> None:
        ...
        for value_map in values:
            for name, value in value_map.items():
                Files.change_var_fun(name, value, system_path, file_name)
    ...
    def run_blockMesh(self, **options) -> None:
        ...
        * **options are the optional arguments. The set of avaible settings are listed below
        ...
        return run_command(["blockMesh"], case_path)

    def run_salome_mesh(self, **options):
        info_key = self.get_key(options.get("info_key"))
        path_key = options.get("key_script_path")
        script_path = Priority.path(
            options.get("script_path"), self.info[info_key], path_key=path_key
        )

        parameter_path = Priority.path(
            options.get("parameter_path"), self.info[info_key], options.get("key_parameter_path")
        )

        script_name = script_path.stem
        script_root_path = script_path.parent

        command = ["salome", "-t", script_name, f"args:{parameter_path}"]
        return run_command(command, script_root_path)
    ...
    def run_decompose(self, what="OF", **options):
        ...
        match what:
            ...
            case "elmer" | "Elmer":
                ...
                command = ["ElmerGrid", "2", "2", str(mesh_name), "-metis", str(cores), "-force"]
                return run_command(command, case_path)
            case "gMesh" | "gmesh":
                case_path = Priority.path(
                    options.get("case_path"), self.info[info_key], path_key="case_path"
                ...
                command1 = ["gmsh", "-3", f"{mesh_name}.geo"]
                command2 = ["ElmerGrid", "14", "2", str(mesh_name), "-autoclean"]
