...
class PostProcess:
    ...
    def rebuildDataRun(
        ...
        values=("U:0",),
        columns=("Points:2",),
        index=("Points:0", "Points:1"),
        methodFill="bfill",
    ):

        source = self.data[key] if data is None else data[key]
        self.rebuild_data[key] = self._pivot_data(
            source, values=values, columns=columns, index=index, fill_method=methodFill
        )
        dataNew = {}
        for value in values:
            dataNew[value] = dict()
            ...
            for xi in dataNew[value]["xnew"]:
                dataNew[value]["value"][xi] = (
                    self.rebuild_data[key][value].xs(xi, level="Points:0").values
                )
                dataNew[value]["ynew"][xi] = (
                    self.rebuild_data[key][value].xs(xi, level="Points:0").axes[0].values
                )
                dataNew[value]["znew"][xi] = (
                    self.rebuild_data[key][value].xs(xi, level="Points:0").axes[1].values
                )
    ...
    def rebuildDataRun2(
        ...
        data=None,
        values=("U:0",),
        generalAxis="Points:0",
        surfaceAxis=("Points:1", "Points:2"),
        methodFill="bfill",
    ):
        """
        ...
        source = self.data[key] if data is None else data
        self.rebuild_data[key] = self._pivot_data(
            source, values=values, columns=columns, index=index, fill_method=methodFill
        )
        dataNew = {}
        for value in values:
            ...
            for xi in dataNew["gAxis"]:
                ...
                dataNew["sAxis2"][xi] = (
                    self.rebuild_data[key][value].xs(xi, level=generalAxis).axes[0].values
                )
                # x for graph
                dataNew["sAxis1"][xi] = (
                    self.rebuild_data[key][value].xs(xi, level=generalAxis).axes[1].values
                )

        self.newData[key] = dataNew

    @staticmethod
    def _pivot_data(data, *, values, columns, index, fill_method):
        """Create and fill a pivot table for both legacy rebuild methods."""
        table = pd.pivot_table(data, values=values, columns=columns, index=index)
        if fill_method == "interpolate":
            return table.interpolate()
        return table.bfill() if fill_method == "bfill" else table.fillna(method=fill_method)
