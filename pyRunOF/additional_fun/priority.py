        ...
        if check_path.parent.exists():  # -> os.path.exists(dir_path):
            if make_new is True:
                ...
                raise_error(
                    "CHECK_PATH_EXIST_1",
                    path=check_path,
                    folder=check_path.stem,
                    dir=check_path.parent,
                )
        else:
            raise_error(
                "CHECK_PATH_EXIST_2", path=check_path, folder=check_path.stem, dir=check_path.parent
            )
        ...
        if var is None:
            if _exist_dict_and_key(info_node, node_key):
                try:
                    ...
                    raise_error("INFO_KEY_ERROR", info_node=info_node, key=node_key)
                else:
                    return info_node[node_key]
            else:
                raise_error(
                    "PRIORITY:NO_CORRECT_ARGS",
                    var=var,
                )
        else:
            return var
...
def _check_type_info_key(key):
    if key is not None and isinstance(key, Hashable):
        ...
        raise_error("INFO:INCORRECT_KEY_TYPE", key=key, key_type=type(key))
...
def _check_type_info(info_node):
    if isinstance(info_node, dict):
        ...
        raise_error("INFO:IS_NOT_DICT", info=info_node, info_type=type(info_node))
...
def _check_path_type(path) -> Any:
    ...
    """
    path_types = (str, os.PathLike, pl.PosixPath, pl.WindowsPath)
